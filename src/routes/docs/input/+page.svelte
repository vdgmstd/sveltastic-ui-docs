<script lang="ts">
	...
	const rootApi: ApiProp[] = [
		...
		{ name: 'iconColor', type: 'Color', required: false, default: null, description: 'When set, the icon background fills with this color on focus and the glyph flips to the ink that reads on that fill (dark on warning / light fills, white on dark ones). When unset, only the glyph color changes on focus.' },
		...
		{ name: 'children', type: 'Snippet', required: false, default: null, description: 'Place Input.Control and Input.Message here — an Input.Label may also sit at this level, and still clears a leading Input.Icon.' }
	];
	...
	const controlApi: ApiProp[] = [
		...
		{ name: 'children', type: 'Snippet', required: false, default: null, description: 'Place Input.Icon, Input.Field, and Input.Label here. Input.Label also works one level up, directly under Input.Root.' }
	];
	...
	const fieldApi: ApiProp[] = [
		...
		{ name: 'placeholder', type: 'string', required: false, default: null, description: "Placeholder text. Under labelStyle='placeholder' with no Input.Label it becomes the floating label instead of being dropped." },
	...
	const iconApi: ApiProp[] = [
		{ name: 'oniconclick', type: '(event: MouseEvent) => void', required: false, default: null, description: 'Fires when the icon is clicked — and promotes the decorative <span> to a real, focusable <button>.' },
		{ name: 'ref', type: 'HTMLButtonElement | HTMLSpanElement | null', required: false, default: '$bindable(null)', description: 'Bindable ref to the icon element — a <button> when oniconclick is set, a <span> otherwise.' },
		{ name: 'child', type: 'Snippet<[{ props }]>', required: false, default: null, description: 'Render-delegation: replaces the icon element with your own. The decorative case hands you span props — no type / disabled.' },
...
<ApiTable
	title="Input.Icon"
	api={iconApi}
	hint="Leading / trailing glyph (side set by Input.Root iconPosition). Pass the icon as children. Decorative by default (an aria-hidden <span>); set oniconclick to get a real <button>, which is when the native button attributes forwarded here apply."
/>
<ApiTable
	title="Input.Label"
	api={labelApi}
